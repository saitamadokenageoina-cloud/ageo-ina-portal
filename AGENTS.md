...
### 9-10. 計算ツールは「上限・下限」を必ず確認する（2026-09 監査で発覚）
厚生年金の標準報酬月額は第1等級88,000円〜**第32等級650,000円**で、それを超える報酬でも保険料は650,000円で頭打ちになる。
`calc.html` にはこの上限が無く、標準報酬139万円で本人負担を **127,185円**（正しくは59,475円）と**2倍以上に過大表示**していた。
- `RM.nenkin.maxHyojun = 650000` を定義し、`Math.min(h, RM.nenkin.maxHyojun)` を計算に適用済み。
- 入力欄の `max="1390000"` は**健康保険**の上限であり、厚生年金の上限ではない。混同しないこと。
- 介護保険・雇用保険には別の規定があるため、この上限を流用しないこと。
- **料率を変えるときは、上限・下限もあわせて確認すること。**

### 9-11. 入力が無効になったら、前回の計算結果を必ず消す（2026-09 監査で発覚）
「エラー表示」と「前回の金額」が同じ画面に並ぶと、利用者は古い金額を有効な結果と誤読する。印刷にも残る。
- `calc.html` の労務費は `parseFloat(...)||300000` で**空欄でも勝手に30万円として計算**していた。既定値の代入は禁止。
- `kyosai_calc.html` の `showError()` は結果カードを隠さず掛金が残っていた。`showError` の先頭で結果を初期化するよう修正済み。
- 新しい計算機能を作るときは「無効な入力→結果はどうなるか」を必ずテストすること。

### 9-12. label に for を一括付与するときは「内包している label」を除外する（作業事故あり）
`<label>` が `<input type="checkbox">` を**内包**している箇所に、直後の別要素のIDを `for` として付けてしまい、
**id重複74件**を発生させて `quality-check.js` が落ちた。一括置換する場合は必ず次を除外条件に入れること。

```javascript
// label の中に入力欄があるなら触らない（既に紐付いているため for は不要）
if (/<(?:input|select|textarea)\b/i.test(labelInnerHtml)) continue;
```

### 9-13. JSは動的に読み込まれる。HTMLのgrepだけで使用状況を判断しない（誤診あり）
`guild-config.js` や `navigation.js` が `document.createElement('script')` で複数のJSを**実行時に読み込む**。
そのためHTMLを検索しただけでは「未使用」と誤判定する（監査時に11件と誤報告→実際は5件だった）。
使用状況はPlaywright等で**実際にページを開いて発生したリクエストを計測**して判断すること。

### 9-14. 停止済みスクリプトは削除せず `_archive/js/` へ退避する
`guild-regression-check.js` が「停止済みスクリプトが再読込されていないこと」をファイル名で監視している。
実体を消すのではなく `_archive/js/` へ移動し、テストの監視対象からは外さないこと。
現在の退避済み：`guild-config-v194.js` / `guild-v195-loader.js` / `guild-admin-editor-v195.js` / `guild-display-hotfix-v197.js`
...
```bash
# 公開前に必ず全6種を通すこと（CIでも同じものが走る）
node scripts/quality-check.js
node scripts/guild-regression-check.js
node scripts/guild-template-check.js
node scripts/guild-update-compat-check.js
node scripts/contrast-hotfix-check.js
node scripts/calc-a11y-check.js   # 計算の上限・無効入力・コントラスト・ラベルを検査
```
